...
const $$SketeletonMovie = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="flex flex-col w-[100%] items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-4xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 relative"> <img class="w-full rounded-t-lg md:w-48 md:rounded-none md:rounded-s-lg object-cover h-[100%]"${addAttribute(movie.poster_path ? `https://image.tmdb.org/t/p/original/${movie.poster_path}` : "https://www.themoviedb.org/assets/2/v4/glyphicons/basic/glyphicons-basic-38-picture-grey-c2ebdbb057f2a7614185931650f8cee23fa137b93812ccb132b9df511df1cfac.svg", "src")} alt=""> <div class="flex flex-col justify-between p-4 leading-normal"> <h5 class="mb-2 text-3xl font-bold tracking-tight text-gray-900 dark:text-white">${movie.title}</h5> <div class="flex gap-0"> ${movie.genres.map((genre) => renderTemplate`${renderComponent($$result, "Genres", $$Genres, { "genres": genre.name })}`)} </div> <p class="mb-3 font-normal text-gray-700 dark:text-gray-300">${movie.tagline}</p> <p class="mb-3 font-normal text-gray-700 dark:text-gray-400/90 text-pretty">${movie.overview}</p> </div> <div class="w-14 absolute -ml-20 top-5 left-full"> <p class="absolute top-[26%] left-[26%] w-fit">${`${rating}%`}</p> <svg viewBox="0 0 36 36"> <path d="M18 2.0845
...
const $$Videos = createComponent(async ($$result, $$props, $$slots) => {
  ...
  console.log(key);
  return renderTemplate`${maybeRenderHead()}<div class="snap-center shrink-0 w-4/5 rounded overflow-hidden shadow-lg"> <iframe width="560" height="315"${addAttribute(`https://www.youtube.com/embed/${key}`, "src")} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe> </div>`;
...
const $$Slider = createComponent(async ($$result, $$props, $$slots) => {
  ...
  let cont = 0;
  return renderTemplate`${maybeRenderHead()}<h2>Videos</h2> <section class="snap-mandatory snap-x flex gap-4 w-1/2 overflow-x-auto"> ${data.map((video) => (cont++, cont < 7 ? renderTemplate`${renderComponent($$result, "Videos", $$Videos, { "key": video.key })}` : null))} </section>`;
...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Post" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="grid mx-auto my-10 justify-items-center gap-9"> ${renderComponent($$result2, "SkeletonMovie", $$SketeletonMovie, { "id": id })} ${renderComponent($$result2, "Slider", $$Slider, { "id": id })} </main> ` })}`;
