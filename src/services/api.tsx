export async function getMovies(page: string) {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/movie/popular?language=es-US&page=${page}`,
    options
  );
...
export async function getPersonById(id: string) {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/person/${id}?language=es-US`,
    options
  );
...
export async function getFunctionsByPerson(id: string) {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/person/${id}/combined_credits?language=es-US`,
    options
  );
...
export async function getCastTv(id: string) {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/tv/${id}/aggregate_credits?language=es-US`,
    options
  );
  const { cast } = await response.json().catch((err) => console.error(err));
  cast;
...
export async function searchMovie(
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/search/${filter}?query=${request}&include_adult=false&language=es-US&page=1`,
    options
  );
  const { results } = await response.json().catch((err) => console.error(err));
  results;
